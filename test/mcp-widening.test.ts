...
const mk = (caps: string[]): FullRecipe => ({
  name: 't', description: '', inputSchema: {}, code: 'export default async () => (await machine.shell({command:"ls"})).stdout',
  capabilities: caps, status: 'enabled', visibility: 'private', version: 1, sourceRunId: null,
