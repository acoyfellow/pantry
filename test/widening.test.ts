...
test('two harnesses with DIFFERENT shell internals produce byte-identical output (E-WIDEN-2)', async () => {
  ...
  const a = await (runRecipe(dfRecipe, { input: {}, bindings: harnessA })
    .output as Promise<unknown>);
  const b = await (runRecipe(dfRecipe, { input: {}, bindings: harnessB })
    .output as Promise<unknown>);
...
test('namespace.method binding resolves via a rebuilt namespace object', async () => {
  const r = recipe(
    `export default async () => { const o = await machine.shell({ command: "echo hi" }); return o.stdout.trim(); }`,
    ['machine.shell'],
  );
  const out = await (runRecipe(r, {
    input: {},
    bindings: { 'machine.shell': async () => ({ stdout: 'hi\n' }) },
  }).output as Promise<unknown>);
  expect(out).toBe('hi');
});
...
test('DANE: namespace-object binding { machine: { shell } } resolves', async () => {
  const r = recipe(`export default async () => (await machine.shell({ command: "ls" })).stdout`, [
    'machine.shell',
  ]);
  const out = await (runRecipe(r, {
    input: {},
    bindings: { machine: { shell: async () => ({ stdout: 'ok' }) } } as any,
  }).output as Promise<unknown>);
  expect(out).toBe('ok');
});
